...
public class Yahtzee {
	public static void main (String[] args) {
		// Add scanner to ask number of players
		int PLAYER_NUM = 2;
		/* Initialize objects or object arrays */
		Yahtzee game = new Yahtzee();
		DiceGroup group = new DiceGroup();
		YahtzeePlayer[] players = new YahtzeePlayer[PLAYER_NUM];
		YahtzeeScoreCard[] cards = new YahtzeeScoreCard[PLAYER_NUM];
		for (int i = 0; i < PLAYER_NUM; i++) {
			players[i] = new YahtzeePlayer();
			cards[i] = new YahtzeeScoreCard();
		}
		// Test
		cards[0].initiateScore();
		game.printHeader();
		group.rollDice();
		group.printDice();
		cards[0].changeScore(6, group);
	}
