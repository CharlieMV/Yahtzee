...
public class YahtzeeScoreCard {
	 ...
	 * 
	 * @ param player	the player object array
	 */
	...
	public void threeOfAKind(DiceGroup dg) {
		...
		for (int i = 0; i < 5; i++)
			numberCount[dg.getDie(i).getLastRollValue() - 1] ++;
	...
	public void largeStraight(DiceGroup dg) {
		...
		if (isStraight) scores[10] = 40;
		else scores[10] = 0;
	}
	
	public void chance(DiceGroup dg) {
		/* Score = sum of all die */
		scores[11] = dg.getTotal();
	}
	
	public void yahtzeeScore(DiceGroup dg) {
		boolean isYahtzee = true;
		/* Check that all die = same value */
		for (int i = 1; i< 5; i++)
			/* isYahtzee = false if at least 1 die doesn't match up */
			if (dg.getDie(i).getLastRollValue() != dg.getDie(0).getLastRollValue());
				isYahtzee = false;
		/* If there is a Yahtzee, add 50, if not, the score is 0 */
		if (isYahtzee) scores[13] = 50;
		else scores[12] = 0;
	}
